...
A continous integration server -- Jenkins -- has been set up @ ci.uwbe.org.in. If you need access to this server, please contact Subha or Sanjay. Jenkins will monitor the GitHub develop branch for any commits. When a new commit is recorded, Jenkins will re-deploy the latest code from 'develop' onto dev.uwbe.org.in. Then, unit tests will be run against this latest codebase. If the tests fail, e-mails will be sent out to the development group and the last person who committed code will have to fix the breakage.
